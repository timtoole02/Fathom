...
- The public risk scan rejects obvious tracked secret-token values such as OpenAI-style, GitHub, Hugging Face, Slack, and bearer tokens while allowing placeholder API-key copy. It also rejects PEM private-key block headers while allowing public-key headers, tracked OS/editor metadata files, tracked credential/config filenames, tracked workspace/personal agent context files, tracked local runtime/artifact detail files, tracked Python cache/build artifacts, tracked frontend/Node cache/build artifacts, tracked symlinks that escape the repository, and tracked files larger than 1 MiB so launch-facing checks catch accidental machine-local noise or large artifacts before publication.
