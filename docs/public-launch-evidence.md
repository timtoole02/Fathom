...
- Latest public-contract QA hardening commit: `1bd28fbf75fb5039bb6d816a9208d0a17d737d63` (`Guard public risk scan IDE artifacts`)
- Scope: no-download public `/v1` contract, public launch checklist/evidence snapshot, manifest-driven smoke, sanitized public-contract smoke artifacts, public-contract smoke duplicate-entry schema guards, public-contract smoke Markdown/status/proof-scope row consistency, failed public-contract smoke endpoint/boundary drift guards, malformed `/v1` JSON request body refusal smoke, unsupported `/v1` route/method public-contract refusal smoke, synthetic PyTorch `.bin` public-contract refusal smoke, unsupported ONNX chat/general public-contract refusal smoke, unverified SafeTensors/Hugging Face public-contract refusal smoke, metadata-only GGUF public-contract refusal smoke, offline public-contract and backend acceptance artifact QA, manifest shape validation, manifest-to-`/v1` docs boundary coverage, public refusal boundary status/code/request-hint coverage, local API no-auth/loopback security-note coverage, API example loopback default guards, REST Client `.http` request-order/header guards and negative self-tests, tracked Python and shell syntax coverage guards, frontend build/UI copy launch-gate guards in the checklist and CI, public security/privacy reporting copy guard, tracked-file secret-token/private-key, OS/editor metadata-file, editor backup/swap artifact guard, IDE workspace/config artifact guard, credential/config filename, workspace/personal agent context-file guard, root `.gitignore` personal workspace context guard, local runtime/artifact detail-file guard, Python cache/build artifact guard, frontend/Node cache/build artifact guard, Rust/Cargo cache/build artifact guard, release/package artifact guard, and symlink escape guards, public risk-scan caveat coverage that preserves that the scanner is not a complete privacy audit, OpenAI SDK example offline request/response regression, API contract issue-template privacy checks, blank public issue disablement checks, bug/security issue-template privacy checks, PR template truthfulness/privacy checks, CI token-permissions guard, CI public risk-scan wiring guard, public overclaim scanner self-test coverage, production-readiness/legal-license public overclaim examples, offline MiniLM/SmolLM2/Qwen2.5 optional API acceptance artifact QA self-tests, CI/static policy wiring, and Phase 16 narrow catalog-backed optional API/runtime evidence for MiniLM embeddings, SmolLM2 135M Instruct, and Qwen2.5 0.5B Instruct.
...
- The public risk scan rejects obvious tracked secret-token values such as OpenAI-style, GitHub, Hugging Face, Slack, and bearer tokens while allowing placeholder API-key copy. It also rejects PEM private-key block headers while allowing public-key headers, tracked OS/editor metadata files, tracked editor backup/swap artifacts, tracked IDE workspace/config artifacts, tracked credential/config filenames, tracked workspace/personal agent context files, missing root `.gitignore` coverage for personal workspace context, tracked local runtime/artifact detail files, tracked Python cache/build artifacts, tracked frontend/Node cache/build artifacts including root-level `dist/` or `build/` output, tracked Rust/Cargo cache/build artifacts, tracked release/package artifacts, tracked symlinks that escape the repository, and tracked files larger than 1 MiB so launch-facing checks catch accidental machine-local noise or large artifacts before publication.
