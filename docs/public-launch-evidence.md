...
- Latest public-contract QA hardening commit: `a5e761cc00b0985562ac83535833c72422ec3e88` (`Assert refusal matrix boundary outcomes`)
- Scope: no-download public `/v1` contract, public launch checklist/evidence snapshot, manifest-driven smoke, sanitized public-contract smoke artifacts, synthetic PyTorch `.bin` public-contract refusal smoke, unsupported ONNX chat/general public-contract refusal smoke, unverified SafeTensors/Hugging Face public-contract refusal smoke, metadata-only GGUF public-contract refusal smoke, offline public-contract artifact QA, manifest shape validation, manifest-to-`/v1` docs boundary coverage, offline MiniLM/SmolLM2/Qwen2.5 optional API acceptance artifact QA self-tests, CI/static policy wiring, and Phase 16 narrow catalog-backed optional API/runtime evidence for MiniLM embeddings, SmolLM2 135M Instruct, and Qwen2.5 0.5B Instruct.
...
- The offline public API contract QA gate validates manifest shape and keeps manifest boundary names covered by the `/v1` docs and refusal matrix.
