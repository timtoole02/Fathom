...
- The offline API client example regression gate keeps the REST Client `.http` example on the expected request sequence and requires JSON `Content-Type` headers on every documented POST block.
