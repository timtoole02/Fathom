...
- Malformed `/v1` JSON request bodies remain predictable refusals: the no-download public contract smoke checks malformed JSON on `POST /v1/chat/completions` returns `400 invalid_request` in the standard JSON error envelope, without emitting fake chat `choices` or embedding `data`.
