...
The offline API client example regression also keeps `docs/api/client-examples.md` aligned with the example script defaults for chat and embedding model ids, catalog repo ids, filenames, prompt/max-token defaults, opt-in embedding flag, and embedding input. It runs cURL, no-deps Python, and OpenAI SDK examples against fake loopback `FATHOM_*` overrides so documented environment overrides keep reaching install, chat, and embedding request bodies. It keeps the cURL quickstart dependency-light and the no-deps Python example on a standard-library import allow-list. It also parses the REST Client `.http` JSON bodies so catalog installs keep reviewed repo/filename pairs, chat bodies keep the `@model` variable with non-empty `messages` and no embedding fields, and embeddings bodies keep the `@embedding_model` variable with `encoding_format: "float"` and no chat fields.
...
The optional `public-contract-smoke-summary.json` and `.md` files include commit, generated UTC timestamp, manifest path/name/status, endpoint checks, boundary checks, and scope caveats only. They intentionally omit local temp paths, server log tails, request secrets, and model/provider payloads. Validate them offline before sharing with `python3 scripts/public_contract_smoke_artifact_qa.py public-contract-artifacts`.
...
It also blocks tracked SonarQube/SonarScanner local analysis outputs such as `.scannerwork/` and `.sonar/`, with root and nested `.gitignore` coverage for those local code-quality scanner artifacts. It does not treat source-of-truth scanner configuration such as `sonar-project.properties` as a local artifact.

It also blocks tracked Semgrep local cache/state directories such as `.semgrep/`, with root and nested `.gitignore` coverage for those local code-quality scanner artifacts. It does not treat source-of-truth Semgrep configuration such as `.semgrep.yml` or `semgrep.yml` as a local artifact.

It also blocks tracked Python static-analysis report outputs from Ruff, Pylint, Mypy, and Pyright such as `ruff-report/`, `ruff-report.json`, `pylint-report.txt`, `mypy-report/`, `mypy-results.xml`, `pyright-report.json`, and timestamped `*-report-*` or `*-results-*` files as local code-quality report artifacts, with matching root and nested `.gitignore` coverage while preserving source-of-truth Python analysis configuration such as `pyproject.toml`, `ruff.toml`, `.ruff.toml`, `.pylintrc`, `mypy.ini`, and `pyrightconfig.json`.

It also blocks tracked TypeScript type-check report outputs such as `tsc-report/`, `tsc-report.txt`, `typecheck-report/`, `typecheck-results.json`, `typescript-report.xml`, and timestamped `tsc-report-*`, `typecheck-results-*`, or `typescript-results-*` files as local code-quality report artifacts, with matching root and nested `.gitignore` coverage while preserving source-of-truth TypeScript project configuration such as `tsconfig.json`, `tsconfig.*.json`, frontend package manifests, and JavaScript/TypeScript source files.
...
It also blocks tracked local frontend bundle-analysis outputs such as `bundle-analyzer-report.html`, `bundle-analysis.json`, `bundle-stats.html`, `webpack-stats.json`, `rollup-stats.html`, and `vite-bundle-visualizer.html`, including timestamped/name-suffixed variants, with matching root and nested `.gitignore` coverage. It preserves source-of-truth bundler configuration and package metadata such as `webpack.config.*`, `rollup.config.*`, `vite.config.*`, and frontend package manifests.
...
It also blocks tracked Axe/Pa11y accessibility audit outputs such as `axe-report/`, `axe-reports/`, `axe-report.html`, `axe-results.json`, `pa11y-report/`, `pa11y-reports/`, `pa11y-ci-report.json`, and `pa11y-results.json`, with matching root and nested `.gitignore` coverage for generated accessibility reports. It does not treat source-of-truth accessibility configuration such as `axe.config.*`, `.pa11yci`, or `pa11yci.json` as a local report artifact.
...
It also blocks tracked local security/dependency scan report outputs such as `snyk-report/`, `snyk-reports/`, `cargo-audit-report/`, `cargo-deny-report/`, `npm-audit-report/`, `pnpm-audit-report/`, `yarn-audit-report/`, `osv-scanner-report/`, `trivy-report/`, `grype-report/`, `dependency-check-report/`, `snyk-report.json`, `cargo-audit-results.xml`, `cargo-deny-results.txt`, `npm-audit.json`, `osv-scanner-results.json`, `trivy-results.json`, `grype-results.txt`, and `dependency-check-report.html`, with matching root and nested `.gitignore` coverage for generated security/dependency scan reports. It does not treat source-of-truth dependency lockfiles or scanner configuration such as `Cargo.lock`, `package-lock.json`, `.snyk`, `deny.toml`, `cargo-deny.toml`, `osv-scanner.toml`, `trivy.yaml`, `grype.yaml`, or `dependency-check.properties` as local report artifacts.

It also blocks tracked generated SBOM/license inventory outputs such as `sbom/`, `sboms/`, `cyclonedx/`, `cyclonedx-reports/`, `license-report/`, `license-reports/`, `licenses-report/`, `licenses-reports/`, `syft-report/`, `syft-reports/`, `sbom.json`, `sbom.spdx.json`, `bom.json`, `cyclonedx.json`, `syft-report.json`, and `license-report.json`, with matching root and nested `.gitignore` coverage for generated SBOM/license inventory outputs. It does not treat source-of-truth SBOM or inventory tool configuration such as `cdxgen.yml`, `.cdxgenrc`, `syft.yaml`, and `.syft.yaml` as local generated inventory output.
...
It also blocks tracked local documentation build artifacts such as LaTeX auxiliary files (`*.aux`, `*.bbl`, `*.blg`, `*.fdb_latexmk`, `*.fls`, `*.synctex.gz`), `_minted-*` caches, and generated API documentation exports such as `swagger-ui/`, `swagger-ui-dist/`, `redoc-static/`, `openapi-generated/`, `api-docs-generated/`, `swagger-ui.html`, `redoc-static.html`, and `openapi-bundle.json`, with matching root and nested `.gitignore` coverage for local documentation build artifacts. It intentionally does not block all PDFs or source-of-truth API specification/configuration files such as `openapi.yaml`, `openapi.json`, `.redocly.yaml`, `redocly.yaml`, `openapitools.json`, and `openapi-generator-config.*` because those can be public docs inputs rather than rebuildable local output.
...
It also blocks tracked LCOV HTML report directories such as `lcov-report/` at any tree depth, with matching root and nested `.gitignore` coverage for LCOV HTML report directories. It does not treat source-of-truth coverage configuration files as local report artifacts.
...
The backend acceptance artifact summaries reject local paths, secret markers, and request/payload text in shareable `summary.json` and `summary.md`; keep `summary.base_url` loopback-only and aligned between `summary.json` and `summary.md`; keep the Markdown Port row aligned with `summary.port` and `summary.base_url`; keep `summary.started_at` and `summary.finished_at` as RFC3339 UTC timestamps aligned between `summary.json` and `summary.md`; keep `summary.md` artifact-index rows aligned with `summary.json` checks; keep `repo_commit` and `fixture_model_ids` aligned between `summary.json` and `summary.md`; keep share-safe artifact/state/model/log/local-path labels aligned between `summary.json` and `summary.md`; keep failed-run diagnostics aligned between `summary.json` and `summary.md`; keep backend acceptance boundary caveats present in `summary.md`; keep full JSON artifacts and logs on the manual review path before publishing.
...
The test report artifact guard also blocks Go test runner report exports such as `gotestsum.json`, `gotestsum-report.xml`, `go-test-report.json`, and `go-test-results.xml` as local test report artifacts, with matching root `.gitignore` coverage for generated Go test report files.
...
The test report artifact guard also blocks generated load-test report outputs from k6, Artillery, and Locust such as `k6-report/`, `k6-reports/`, `k6-report.html`, `k6-summary.json`, `artillery-report/`, `artillery-results.json`, `locust-report/`, and `locust_stats.csv`, with matching root and nested `.gitignore` coverage while preserving source-of-truth test definitions such as k6 scripts, Artillery YAML, and `locustfile.py`.

The test report artifact guard also blocks Robot Framework result/report outputs such as `robot-results/`, `robot-reports/`, `robot-output.xml`, `robot-log.html`, and `robot-report.html` as local test report artifacts, with matching root and nested `.gitignore` coverage for directory-shaped Robot outputs and root `.gitignore` coverage for explicitly Robot-named report files. It intentionally does not block generic default names such as `output.xml`, `log.html`, or `report.html` outside Robot-specific output directories.
...
The test report artifact guard also blocks Jest generated report outputs such as `jest-stare/`, `jest-html-reporters-attach/`, `jest_html_reporters.html`, `jest-junit.xml`, and `jest-results.json` as local test report artifacts, with matching root and nested `.gitignore` coverage for directory-shaped Jest reports and root `.gitignore` coverage for explicitly Jest-named report files. It does not block source-of-truth Jest configuration files such as `jest.config.js`.

The test report artifact guard also blocks Vitest generated report outputs such as `vitest-report/`, `vitest-report.html`, `vitest-report.json`, `vitest-report-*` variants, `vitest-junit.xml`, and `vitest-results.json` as local test report artifacts, with matching root and nested `.gitignore` coverage for directory-shaped Vitest reports and root `.gitignore` coverage for explicitly Vitest-named report files. It does not block source-of-truth Vitest configuration files such as `vitest.config.js` or `vitest.config.ts`.
...
The test report artifact guard also blocks Cobertura/OpenCover coverage XML outputs (`cobertura.xml`, `*.cobertura.xml`, `opencover.xml`, and `*.opencover.xml`) as local test report artifacts, with matching root `.gitignore` coverage.
