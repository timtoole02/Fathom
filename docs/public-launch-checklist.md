...
It also blocks tracked local frontend framework caches and build outputs such as Astro `.astro/`, Docusaurus `.docusaurus/`, VitePress `.vitepress/cache/` and `.vitepress/dist/`, Next.js `.next/`, SvelteKit `.svelte-kit/`, Nuxt `.nuxt/` and `.output/`, and Angular `.angular/cache/` directories, with matching root `.gitignore` coverage.
