...
It also blocks tracked local frontend/static-site framework caches and build outputs such as Astro `.astro/`, Docusaurus `.docusaurus/`, Hugo `resources/_gen/` and `.hugo_build.lock`, Jekyll `_site/`, `.jekyll-cache/`, and `.sass-cache/`, VitePress `.vitepress/cache/` and `.vitepress/dist/`, Next.js `.next/`, SvelteKit `.svelte-kit/`, Nuxt `.nuxt/` and `.output/`, and Angular `.angular/cache/` directories, with matching root `.gitignore` coverage.
