...
def assert_boundary_docs() -> None:
    ...
    assert_contains(
        evidence_text,
        "tracked local native/CMake build artifacts such as `cmake-build-*`, `CMakeFiles/`, `CMakeCache.txt`, `cmake_install.cmake`, `compile_commands.json`, and Ninja local state files such as `.ninja_deps` and `.ninja_log`",
