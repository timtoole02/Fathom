...
def assert_boundary_docs() -> None:
    ...
    assert_contains(
        evidence_text,
        "tracked local native/CMake build/user-local artifacts such as `cmake-build-*`, `CMakeFiles/` at any tree depth, CTest `Testing/Temporary/` output, generated CTest/Dart files such as `CTestTestfile.cmake` and `DartConfiguration.tcl`, generated CPack files such as `CPackConfig.cmake` and `CPackSourceConfig.cmake`, `CMakeCache.txt`, user-local `CMakeUserPresets.json`, `cmake_install.cmake`, `compile_commands.json`, `install_manifest.txt`, and Ninja local state files such as `.ninja_deps` and `.ninja_log`",
