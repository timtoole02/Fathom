...
if [[ "${FATHOM_QWEN25_ACCEPTANCE:-}" != "1" ]]; then
  cat >&2 <<'EOF'
Qwen2.5 optional API acceptance is opt-in because it downloads or reuses about 1 GB of model files and can use several GB of RAM.
Set FATHOM_QWEN25_ACCEPTANCE=1 to run it.
EOF
  exit 2
fi
...
PORT="${FATHOM_QWEN25_ACCEPTANCE_PORT:-18185}"
BASE="http://127.0.0.1:${PORT}"
WAIT_SECONDS="${FATHOM_QWEN25_ACCEPTANCE_WAIT_SECONDS:-240}"
REQUEST_TIMEOUT="${FATHOM_QWEN25_ACCEPTANCE_REQUEST_TIMEOUT:-900}"
KEEP_ARTIFACTS="${FATHOM_QWEN25_ACCEPTANCE_KEEP_ARTIFACTS:-0}"
TMP_ROOT="${FATHOM_QWEN25_ACCEPTANCE_ROOT:-${TMPDIR:-/tmp}/fathom-qwen25-api-$$}"
MODELS_DIR="${FATHOM_QWEN25_ACCEPTANCE_MODELS_DIR:-$TMP_ROOT/models}"
STATE_DIR="${FATHOM_QWEN25_ACCEPTANCE_STATE_DIR:-$TMP_ROOT/state}"
ARTIFACT_DIR="${FATHOM_QWEN25_ACCEPTANCE_ARTIFACT_DIR:-$TMP_ROOT/artifacts}"
LOG_DIR="${FATHOM_QWEN25_ACCEPTANCE_LOG_DIR:-$TMP_ROOT/logs}"
...
cleanup() {
  ...
  if [[ $status -eq 0 && "$KEEP_ARTIFACTS" != "1" && -z "${FATHOM_QWEN25_ACCEPTANCE_ROOT:-}" && -z "${FATHOM_QWEN25_ACCEPTANCE_ARTIFACT_DIR:-}" ]]; then
    rm -rf "$TMP_ROOT"
  else
    echo "Qwen2.5 optional acceptance artifacts: $ARTIFACT_DIR" >&2
...
if curl -fsS "$BASE/v1/health" >/dev/null 2>&1; then
  echo "Port $PORT already has a responding Fathom backend. Choose another FATHOM_QWEN25_ACCEPTANCE_PORT." >&2
  exit 1
fi

mkdir -p "$MODELS_DIR" "$STATE_DIR" "$ARTIFACT_DIR" "$LOG_DIR"

echo "== start isolated Fathom server for optional Qwen2.5 API acceptance =="
FATHOM_PORT="$PORT" \
...
save("01-v1-health.json", status, health, "health", "Backend health responds before optional Qwen2.5 install.", 200)
assert status == 200, health

status, install = request("POST", "/api/models/catalog/install", {"repo_id": REPO_ID, "filename": "model.safetensors"})
save("02-install-qwen25.json", status, install, "install_qwen25", "Pinned Qwen2.5 catalog demo installs through catalog verification.", 200)
...
save("03-v1-models-after-qwen25.json", status, models, "models_include_qwen25", "Validated Qwen2.5 demo appears in /v1/models after install.", 200)
...
chat_body = {
    "model": MODEL_ID,
    "messages": [{"role": "user", "content": "Reply with one short sentence about local inference."}],
    "max_tokens": 24,
    "temperature": 0.2,
}
for artifact, check, description in [
    ("04-chat-cold.json", "chat_cold_qwen2", "First same-process Qwen2.5 chat call returns real local content and metrics."),
    ("05-chat-warm.json", "chat_warm_qwen2", "Second same-process Qwen2.5 chat call returns real local content and warm metrics."),
]:
    ...
    assert metrics.get("runtime_family") == "qwen2", metrics
...
save("06-chat-stream-refusal.json", status, stream, "stream_refusal", "Streaming remains truthfully refused for Qwen2.5.", 501)
...
summary = {
    ...
    "caveats": [
        ...
        "Does not prove generation quality, latency, throughput, production readiness, legal suitability, broad Qwen-family compatibility, arbitrary Hugging Face execution, streaming, external proxying, or full OpenAI API parity.",
...
(artifacts / "summary.md").write_text(
    ...
    "- No generation quality, latency, throughput, production readiness, legal suitability, broad Qwen-family compatibility, arbitrary Hugging Face execution, streaming, external proxying, or full OpenAI API parity claim.\n"
...
python3 scripts/qwen25_optional_api_acceptance_artifact_qa.py "$ARTIFACT_DIR"

echo "Qwen2.5 optional API acceptance passed. Artifacts: $ARTIFACT_DIR"
