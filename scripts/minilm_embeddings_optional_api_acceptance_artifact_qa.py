...
def assert_markdown_timestamps_match_summary(summary:dict[str,Any],md:str)->None:
    for key,label in (('started_at','Started'),('finished_at','Finished')):
        value=summary.get(key)
        if isinstance(value,str) and value and f"- {label}: `{value}`" not in md:
            raise AssertionError(f'summary.md missing {key} row matching summary.json')

def assert_markdown_identity_matches_summary(summary:dict[str,Any],md:str)->None:
...
def assert_markdown_path_labels_match_summary(summary:dict[str,Any],md:str)->None:
    for key,label in (('artifact_dir','Artifact directory'),('state_dir','State directory'),('model_dir','Model directory')):
        value=summary.get(key)
        if not isinstance(value,str) or not value:
            raise AssertionError(f'summary.{key} must be non-empty text')
        if f"- {label}: `{value}`" not in md:
    ...
    if f"- Server log: `{server_log}`" not in md:
...
def main():
    ...
    if not dirs:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bad_md_timestamp=Path(tmp)/'bad-markdown-timestamp'; write_sample(bad_md_timestamp)
            (bad_md_timestamp/'summary.md').write_text((bad_md_timestamp/'summary.md').read_text().replace('- Finished: `2026-04-29T00:00:01Z`\n','- Finished: `2026-04-29T00:00:02Z`\n'))
            try:
                validate_summary(bad_md_timestamp)
            except AssertionError as exc:
                if 'summary.md missing finished_at row' not in str(exc): raise
            else:
                raise AssertionError('bad summary.md timestamp self-check did not fail')
            bad_md_commit=Path(tmp)/'bad-markdown-commit'; write_sample(bad_md_commit)
            (bad_md_commit/'summary.md').write_text((bad_md_commit/'summary.md').read_text().replace('- Repo commit: `sample`\n','- Repo commit: `stale`\n'))
            try:
            ...
            bad_md_path_label=Path(tmp)/'bad-markdown-path-label'; write_sample(bad_md_path_label)
            (bad_md_path_label/'summary.md').write_text((bad_md_path_label/'summary.md').read_text().replace('- Server log: `logs/server.log`\n','- Server log: `stale-logs/server.log`\n'))
            try:
                validate_summary(bad_md_path_label)
            except AssertionError as exc:
                if 'summary.md missing log_dir server-log label' not in str(exc): raise
            else:
