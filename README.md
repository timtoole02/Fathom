...
| Capability reporting | `/api/capabilities`, model inspection, backend-lane summaries, and UI copy distinguish runnable, metadata-readable, planned, blocked, and unsupported states. GGUF files can expose bounded header/key-value metadata, tensor descriptors, validated internal payload ranges, tokenizer/architecture compatibility hints, internal bounded tokenizer metadata retention for narrow synthetic GPT-2/BPE, exact-size Llama 3 BPE, and Llama/SentencePiece shapes, private fixture-scoped tokenizer parity helpers, and internal synthetic payload decode checks without claiming public/runtime tokenizer execution or inference. |
...
The narrow `/v1` contract is documented in [`docs/api/v1-contract.md`](docs/api/v1-contract.md), with a small machine-checkable public launch API contract manifest at [`docs/api/public-contract.json`](docs/api/public-contract.json). Backend/API examples for health, capabilities, catalog install, `/v1/models`, `/v1/chat/completions`, explicit-vector retrieval, and embedding endpoints live in [`docs/api/backend-only-quickstart.md`](docs/api/backend-only-quickstart.md). The optional MiniLM embedding, SmolLM2, and Qwen2.5 demos have separate opt-in API acceptance recipes at [`docs/api/minilm-embeddings-optional-acceptance.md`](docs/api/minilm-embeddings-optional-acceptance.md), [`docs/api/smollm2-optional-acceptance.md`](docs/api/smollm2-optional-acceptance.md), and [`docs/api/qwen25-optional-acceptance.md`](docs/api/qwen25-optional-acceptance.md). Minimal client examples for cURL, dependency-free Python, the OpenAI Python SDK, and `.http` editors live in [`docs/api/client-examples.md`](docs/api/client-examples.md). The public launch checklist in [`docs/public-launch-checklist.md`](docs/public-launch-checklist.md) ties together clean clone/install, no-download verification, backend/API smoke, optional acceptance artifacts, and troubleshooting. Security posture and local threat-model notes live in [`SECURITY.md`](SECURITY.md); contribution truthfulness and verification expectations live in [`CONTRIBUTING.md`](CONTRIBUTING.md). To verify the public launch API boundary without downloads, run `bash scripts/public_api_contract_smoke.sh`; it starts the real backend with isolated temporary state/model directories and checks routing/refusal envelopes only, not model quality or runtime acceptance. If you save public-contract smoke handoff summaries with `FATHOM_PUBLIC_CONTRACT_ARTIFACT_DIR`, validate them offline with `python3 scripts/public_contract_smoke_artifact_qa.py <artifact-dir>` before sharing. For a fuller optional networked backend-only acceptance pass with isolated state/model directories, run `FATHOM_ACCEPTANCE_KEEP_ARTIFACTS=1 bash scripts/backend_acceptance_smoke.sh`; it includes catalog license metadata/acknowledgement-gating evidence before the first fixture download and external API placeholder refusal evidence without calling any external provider. Before sharing generated artifacts publicly, run `bash scripts/public_risk_scan.sh` and review logs/full JSON payloads for local paths or request text; the scanner is a tracked-file guardrail, not a complete privacy audit.
...
For the exact supported `/v1` request/response shapes, see [`docs/api/v1-contract.md`](docs/api/v1-contract.md). For minimal copy-paste client examples and the adopter checklist, see [`docs/api/client-examples.md`](docs/api/client-examples.md) and the scripts in [`examples/api/`](examples/api/). The launch-supported surface is intentionally small: `GET /v1/health`, `GET /v1/models`, non-streaming `POST /v1/chat/completions`, and float-only `POST /v1/embeddings`.
...
Only local models Fathom has actually validated as chat-runnable appear in `/v1/models`. Connected external OpenAI-compatible entries are metadata placeholders only until proxying exists, so they are excluded too. The pinned GGUF fixture can be installed for metadata/provenance inspection, including private bounded retention of narrow Llama/SentencePiece-shaped tokenizer metadata and private fixture-scoped Llama/SentencePiece encode/decode parity helpers. Exact-size Llama 3 BPE retention/parity scaffolding is internal evidence only and is not a public tokenizer or runtime claim. GGUF remains metadata-only, excluded from `/v1/models`, and `/v1/chat/completions` refuses it rather than attempting public/runtime tokenizer execution or generation:
